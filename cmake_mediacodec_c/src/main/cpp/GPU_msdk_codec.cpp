...
struct MSDKEncoder
{
    VM_MSDKEncoder   *pGPUEncoder;
};

struct MSDKEncoder *CreateEncoder(MSdkInputParam *InputParam)
{
    struct MSDKEncoder *pMEncoder = NULL;
    pMEncoder = new struct MSDKEncoder;
    pMEncoder->pGPUEncoder = VM_MSDKEncoder::CreateEncoder(InputParam);
    return pMEncoder;
}

/////////////////////////////////////////////////////////////////////////////////////
void DeleteEncoder(MSDKEncoder *pMEncoder)
{
    //Delete the Intel MSDK encoder and release memory.
    if (pMEncoder != NULL)
    {
        pMEncoder->pGPUEncoder->DeleteEncoder(pMEncoder->pGPUEncoder);
    }
    delete pMEncoder;
}

int32_t EncodeFrame(MSDKEncoder *pMEncoder,SSourcePicture* pSrcPic, SLayerBSInfo* pBsLayer)
{
    if (pMEncoder != NULL)
    {
        return pMEncoder->pGPUEncoder->EncodeFrame(pSrcPic, pBsLayer);
    }
    return -1;
}

int32_t GetBitstream(MSDKEncoder *pMEncoder,SLayerBSInfo* pBsLayer)
{
    if (pMEncoder != NULL)
    {
        return pMEncoder->pGPUEncoder->GetBitstream(pBsLayer);
    }
    return -1;
}

int32_t UpdateBitrate(MSDKEncoder *pMEncoder,uint32_t Bitrate, uint32_t Framerate)
{
    if (pMEncoder != NULL)
    {
        return pMEncoder->pGPUEncoder->UpdateBitrate(Bitrate, Framerate);
    }
    return -1;
}

int32_t InsertKeyFrame(MSDKEncoder *pMEncoder)
{
    if (pMEncoder != NULL)
    {
        return pMEncoder->pGPUEncoder->InsertKeyFrame();
    }
    return -1;
